...
/*   Updated: 2024/02/08 16:48:15 by amarna           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* IRC protocol line terminator (carriage return + line feed). */
# define CLRF	"\r\n"
...
/* Handles execution of all IRC commands (PASS, NICK, USER, JOIN, MODE, etc.). */
class command {
	...
	public:
		...
		/* Validate the password sent by client against server password. */
		std::string		PASS(int fd, Server *serv);
		/* Set or update the nickname of the client. */
		std::string		NICK(int fd, Server *serv);
		/* Register the client's username, mode, hostname and realname. */
		...
		std::string		OPER();
		/* Close the client connection and remove it from the server. */
		std::string		QUIT(int fd, Server* serv);
		std::string		PART();
		std::string		TOPIC();
		std::string		KICK();
		/* Handle MODE command: check channel operator privileges. */
		int				MODE(client *client1, Server *serv);
		std::string		PRIVMSG();
		std::string		NOTICE();
		std::string		KILL();
		std::string		WALLOPS();
		/* Add client to an existing channel or create a new one. */
		int		JOIN(client *client1, Server *serv);

		std::string		bot();

		/* Strip trailing \r from each string in the vector (IRC line endings). */
