...
/* Return true if any string in strVector contains at least one alphanumeric character. */
...
/* Convert a positive integer to a string of at most 4 digits (used for temporary client IDs). */
...
/* Return the current date and time as a string (used in RPL_CREATED welcome message). */
...
/* Initialise the t_serv struct: zero-out socket addresses, set IPv4 and port, clear fd_sets. */
...
/* Populate the list of known IRC commands (and their reversed aliases used for partial input). */
...
/* Create TCP socket, bind to port, start listening, and add fd to the select pool. */
...
/* Accept a pending TCP connection and trigger first-connection handling. */
...
/* Handle a brand-new client: read initial commands, validate NICK/USER/PASS, send welcome. */
...
/* Read raw data from socket, split by '\n' into M_requestVector, then build M_cmdMap. */
...
/* Split the raw buffer by '\n' and push each line into M_requestVector. */
...
/* Parse M_requestVector into M_cmdMap: key = command name, value = argument tokens. */
...
/* Split string s by delimiter into a vector of tokens using getline. */
...
/* Iterate M_cmdMap, match each command against the known command list, and execute it. */
...
/* Dispatch execution to the right IRC command handler based on its index in M_commands. */
std::string	Server::executeCmd(int i, int clientFd)
{
	...
	switch (i)
	{
		case 1 :
		{
			...
			// :haha!amarna@localhost NICK bob
...
/* Handle an incoming message from an already-connected client socket. */
...
/* Main select() loop: dispatch new connections or incoming data to the right handler. */
...
/* Register signal handlers: index 1 = SIGINT (graceful shutdown), index 2 = SIGPIPE. */
